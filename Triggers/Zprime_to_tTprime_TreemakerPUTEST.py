...
class Zp_tTp_Treemaker:
	def __init__(self, name, TreeFolder, mc, tt, weight, choose, PU): # Initialize: mc is an important option and data will crash if run with it. Weight should be xs/NevtGen.
		self.mc = mc
		self.tt = tt
		self.name = name
		self.w = weight
		...
		self.puf_npv = self.puf.Get("DataPileupHist") #npv from PU_weight
		self.sadnumber = 0
	...
	def __book__(self): # Create the trees/branches which we will fill
		...
		self.PU_NPV = self.puf_npv.Clone("DataPileupHist")
		...
		self.NPV = array('f', [0.0])
		self.addBranch('pu_NtrueInt', self.NPV, self.tree1)
		self.addBranch('pu_NtrueInt', self.NPV, self.tree2)
	...
	def doAK4JetsMC(self):
		...
		for j in range(len(self.Tree.jetAK4Puppi_Pt)):
			...
			if iJet.Pt() > 45. and iJet.DeltaR(self.TAG) > 0.8:
	...
	def doAK8PartMC(self):
		if self.Tree.jetAK8Puppi_softDropMass[0] > 50 and self.Tree.jetAK8Puppi_Pt[0] > 150 and self.Tree.jetAK8Puppi_tau2[0]>0 and self.Tree.jetAK8Puppi_tau1[0]>0:
			self.Type1 = False
			...
			if self.Tree.jetAK8Puppi_softDropMass[0] > 70 and self.Tree.jetAK8Puppi_Pt[0] > 275:
				self.Type1 = True
			return True
		return False
	def doAK8Part(self):
	...
	def doLepPart(self): # Fill leptoni information and retain self.LEP for further analysis.
		...
		if len(self.Tree.mu_Pt) > 0 and len(self.Tree.el_Pt) > 0: # Need to pick a lepton
			if self.Tree.mu_Pt[0] > self.Tree.el_Pt[0]: # It's a muon
				self.leptype = "mu"
			else: # It's an electron
				self.leptype = "el"
	...
	def FillLepton(self, leptype): # Cut leptons that don't have good pT or eta. Fill variables if they're ok.
		...
		return False
	def doTrigger(self):
		if self.Tree.HLT_Mu45_eta2p1 > 0:
			return True
		if self.Tree.HLT_Ele45_CaloIdVT_GsfTrkIdT_PFJet200_PFJet50 > 0:
			return True
		return False
	def __del__(self):
