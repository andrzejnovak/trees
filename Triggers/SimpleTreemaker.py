...
class Zp_tTp_Treemaker:
	def __init__(self, name, TreeFolder, mc, tt, weight, choose, PU, saveto, trigg=""): # Initialize: mc is an important option and data will crash if run with it. Weight should be xs/NevtGen.
		self.mc = mc
		self.tt = tt
		self.name = name
		self.w = weight
	...
	def FillLepton(self, leptype): # Cut leptons that don't have good pT or eta. Fill variables if they're ok.
		...
		return False
	def doTrigger(self):
		if eval("self.Tree."+self.trigg) > 0:
			return True
		#if eval("self.Tree."+self.trigg)  > 0: 
		#	return True
		return False
